...
export const meta: MetaFunction = ({ location }) => {
  const title = 'PILOT · Partneram Hero Preview';
  const description = 'Preview pilot v4-style hero для /partneram — НЕ для production.';
...
type Requisite = { label: string; value: string; link?: string };
  ...
  return (
    <>
      <div className="bg-accent text-bg-deep px-6 py-2 text-center text-xs font-mono tracking-widest uppercase">
        PILOT PREVIEW · НЕ для production · /pilot-partneram
      </div>

      <PartnerHero
        eyebrow="Розділ 05 · партнерство"
        title="Юридичний контур забудовника"
        lead="Юридична картка справа — повний пакет реквізитів. Деталі для due diligence (документи по проектах, ліцензії, експертиза) — за запитом."
      >
